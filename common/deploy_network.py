...
from image_utils import rescale_intensity # type: ignore
        ...
        for data in data_list:
            ...
            if FLAGS.process_seq:
                ...
                for fr in ['ED', 'ES']:
                    ...
                    """ ## Transformações com a imagem
                    # Aumentando brilho da imagem
                    brilho = 1.3
                    image = np.clip(image * brilho, 0, 255)
                    # Rotacionando a imagem
                    image = rotate(image, angle=-90, axes=(0, 1), reshape=False)
                    # Salvando a imagem transformada
                    transformed_image_path = os.path.join(data_dir, 'transformed_image.nii.gz')
                    nim_transformed = nib.Nifti1Image(image, nim.affine)
                    nib.save(nim_transformed, transformed_image_path)
                    print('  Transformed image saved to {}'.format(transformed_image_path)) """
