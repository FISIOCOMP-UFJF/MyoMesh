...
def _read_msh_robust(path, attempts=4):
    """
    Read a .msh with meshio, tolerating an intermittent IndexError/ValueError in
    the Gmsh ASCII parser (a transient blank/short line leaves meshio with an
    empty token list). The file on disk is well-formed, so simply retrying the
    read succeeds. Raises the last error if every attempt fails.
    """
    import time
    last_err = None
    for i in range(attempts):
        try:
            return meshio.read(path, file_format="gmsh")
...
def request_functions(pathMesh, meshname, carpOutput, aux_alpha_endo_lv, aux_alpha_epi_lv, aux_beta_endo_lv,
    ...
    gmsh_mesh = _read_msh_robust(pathMesh)
    ...
    for i in range(len(peso_arr)):
        if tecido_arr[i] == 0:      # healthy
            peso_arr[i] = 1.0
        elif tecido_arr[i] == 1:    # fibrosis/core
            peso_arr[i] = 0.0
    ...
    with df.XDMFFile(mesh.mpi_comm(), meshname + ".xdmf") as xdmf:
        ...
        xdmf.write(u, 0)
        xdmf.write(region_id, 0)   # <- writing region without error
        xdmf.write(peso_tecido, 0) # NEW: add gray zone resolved with Laplace
...
def convert_xdmf_to_vtu(meshname):
    """Convert the XDMF file generated by FEniCS to VTU, the format required by HexaMeshFromVTK."""
    print(50*"=", flush = True)
...
def embed_surfaces_in_vtu(vtu_path, output_path, region_id_key="region_id"):
    """
    Read a VTU with tetrahedra tagged by region_id, compute the boundary faces
    of each region and add them to the file as triangle cells, keeping the
    corresponding region_id. The result is saved to output_path.
    """
